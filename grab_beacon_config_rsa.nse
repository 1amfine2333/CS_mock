...
local function grab_beacon(response)
    ...
    if (response.status == 200) then
        if (http.response_contains(response, test_string, true)) then
            ...
            -- output["Unknown1"] = parse_field("Unknown1", repacked, "\x00\x04\x00\x02\x00\x04", ">I")
            -- output["Unknown3"] = parse_field("Unknown3", repacked, "\x00\x0b\x00\x03\x01\x00", ">I")
            -- output["Unknown4"] = parse_field("Unknown4", repacked, "\x00\x1c\x00\x02\x00\x04", ">I")
            -- output["Unknown5"] = parse_field("Unknown5", repacked, "\x00\x1f\x00\x01\x00\x02", ">I")
...
action = function(host, port)
    ...
    -- https://nmap.org/nsedoc/lib/json.html

    -- 输出json格式 object 类型
    -- json_output = json.generate(json_output)

    -- json_output = json.make_array(json_output)

    -- json_output = json.make_object(json_output)
    ...
    -- =>> object
    -- json_output = json.typeof(json_output)
