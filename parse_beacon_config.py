...
class cobaltstrikeConfig:
    ...
    def _parse_config(self, version, quiet=False, as_json=False):
        ...
        for conf_name, packed_conf in settings:
            ...
            # 获取正确的公钥长度值，AB结尾
            if conf_name == "PublicKey":
                PublicKey_str = base64.b64encode(parsed_config[conf_name]).decode('UTF-8')
                # print(PublicKey_str)
                PublicKey_ABstr = PublicKey_str.rfind("AB") + 2
                PublicKey_str = PublicKey_str[0 : PublicKey_ABstr]  # 0-216
                parsed_config[conf_name] = base64.b64decode(PublicKey_str.encode("UTF-8"))
